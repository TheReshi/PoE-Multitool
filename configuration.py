...
def create_h_stash_coords():
    ...
    for i in range(1, 49, 2):
        for j in range(1, 49, 2):
            coords.append((round(int(config["STASH"][f"start_x{inv_suffix}"]) + step_x * j), round(int(config["STASH"][f"start_y{inv_suffix}"]) + step_y *  i)))
...
def create_h_inventory_coords():
    ...
    for i in range(1, 11, 2):
        for j in range(1, 25, 2):
            coords.append((round(int(config["INVENTORY"][f"start_x{inv_suffix}"]) + step_x * j), round(int(config["INVENTORY"][f"start_y{inv_suffix}"]) + step_y *  i)))
...
h_inventory_coords = create_h_inventory_coords()
stash_coords = create_stash_coords()
h_stash_coords = create_h_stash_coords()